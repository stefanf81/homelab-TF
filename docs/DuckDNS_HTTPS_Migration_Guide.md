...
*   **Certificate (`certificate.yaml`)**: Requested a certificate for `jokelab.dev` which automatically provisions the `taskflow-tls-secret`.
...
*   **Routing Priority (`httproute.yaml` & `routes.yaml`)**: 
    *   Added `hostnames: ["jokelab.dev"]` to the main `taskflow-route`.
...
*   **CORS (`configmap.yaml`)**: Whitelisted `https://jokelab.dev` so the Angular frontend can successfully query the Spring Boot backend.
...
**Symptom**: Let's Encrypt validation failed because `cert-manager`'s internal self-check pod timed out trying to reach `jokelab.dev`.
...
2.  Added `192.168.50.201 jokelab.dev` to the `NodeHosts` block.
...
**Symptom**: Let's Encrypt reported `DNS problem: query timed out looking up A for jokelab.dev`.
...
*   **Device level**: Edit `/etc/hosts` (Mac/Linux) or `C:\Windows\System32\drivers\etc\hosts` (Windows) and append: `192.168.50.201 jokelab.dev`.

...
*   **Certificate (`certificate.yaml`)**: Requested a certificate for `jokerlab.dev` which automatically provisions the `taskflow-tls-secret`.
...
*   **Routing Priority (`httproute.yaml` & `routes.yaml`)**: 
    *   Added `hostnames: ["jokerlab.dev"]` to the main `taskflow-route`.
...
*   **CORS (`configmap.yaml`)**: Whitelisted `https://jokerlab.dev` so the Angular frontend can successfully query the Spring Boot backend.
...
**Symptom**: Let's Encrypt validation failed because `cert-manager`'s internal self-check pod timed out trying to reach `jokerlab.dev`.
...
2.  Added `192.168.50.201 jokerlab.dev` to the `NodeHosts` block.
...
**Symptom**: Let's Encrypt reported `DNS problem: query timed out looking up A for jokerlab.dev`.
...
**Resolution**: 
1.  Waited 2 minutes.
2.  Verified global propagation using `nslookup jokerlab.dev 1.1.1.1` (Cloudflare).
...
*   **Device level**: Edit `/etc/hosts` (Mac/Linux) or `C:\Windows\System32\drivers\etc\hosts` (Windows) and append: `192.168.50.201 jokerlab.dev`.
